/*!
 *****************************************************************************
 @file:    AD5940_SequencerTEST.c
 @author:  Neo Xu (andriandreo's edit)
 @brief:   Basic usage of sequencer combined with ADC MUX.
           Direct cmd buffer writes instead of SEQ generator. 
 -----------------------------------------------------------------------------
...
static void AppADCSeqGen(void)
{
  ...
  /* Set new MUX Inputs */
  tempreg = AD5940_ReadReg(REG_AFE_ADCCON); // Check parameters
  tempreg &= ~(BITM_AFE_ADCCON_MUXSELN|BITM_AFE_ADCCON_MUXSELP);
  tempreg |= ADCMUXP_VRE0<<BITP_AFE_ADCCON_MUXSELP;
  tempreg |= ADCMUXN_AIN0<<BITP_AFE_ADCCON_MUXSELN;
  Seq0Commands[0] = SEQ_WR(REG_AFE_ADCCON,tempreg); // Write bits to ADCCON register (to set MUX inputs)
  /* ENABLE ADC and Filters */
  tempreg = AD5940_ReadReg(REG_AFE_AFECON); // Check parameters
  tempreg |= AFECTRL_ADCPWR|AFECTRL_SINC2NOTCH;
  Seq0Commands[1] = SEQ_WR(REG_AFE_AFECON,tempreg); // Write bits to AFECON register (to enable ADC and Filters)
  
  Seq0Commands[2] = SEQ_WAIT(16*250); // wait 250us

  /* Start ADC convert */
  tempreg |= AFECTRL_ADCCNV;
  Seq0Commands[3] = SEQ_WR(REG_AFE_AFECON,tempreg); // Write bits to AFECON register (to start ADC conversion)

  Seq0Commands[4] = SEQ_WAIT(WaitClks); // wait for first data ready

  /* Stop ADC */
  tempreg &= ~(AFECTRL_ADCPWR|AFECTRL_ADCCNV|AFECTRL_SINC2NOTCH);
  Seq0Commands[5] = SEQ_WR(REG_AFE_AFECON,tempreg); // Write bits to AFECON register (to stop ADC)

  //AD5940_EnterSleepS();

  Seq0Commands[6] = SEQ_INT0(); //[!!!]   /* Generate custom-interrupt 0. We can generate any custom interrupt(SEQ_INT0/1/2/3()) by sequencer. */
}
...
void AD5940_Main(void)
{
  ...
  seqinfo3.SeqRamAddr = seqinfo2.SeqRamAddr;
